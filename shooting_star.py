...
#보스 공격 구현 (원래는 암석이었는데 보스 공격으로 바꿔서 읽는 데 헷갈릴 수 있음 주의)
class Rock(pygame.sprite.Sprite):    
    def __init__(self, xpos, ypos, speed):
        super(Rock, self).__init__()
        #rock_images = ('rock01.png','rock02.png','rock03.png','rock04.png','rock05.png',\
            #'rock06.png','rock07.png','rock08.png','rock09.png','rock10.png')
        self.image = pygame.image.load('monmissile.png')
...
def game_loop():
    ...
    rocks = pygame.sprite.Group() # 보스 공격
    heals = pygame.sprite.Group() # 회복 아이템
    lazers = pygame.sprite.Group() # 레이저(스킬) 

    occur_prob = 25 # 확률적으로 얼만큼 나오게 할 것인가 (낮을수록 보스가 더 자주 공격함)
    shot_count = 0 # 점수
    count_life = 3 # 목숨
    boss_hp = 100 # 보스 hp
    ...
    while not done:
        ...
        occur_of_rocks = 2 + int((100-boss_hp) / 100)
        min_rock_speed = 2 + int((100-boss_hp) / 50)
        max_rock_speed = 2 + int((100-boss_hp) / 25)
        occur_of_heals = 1
        ...
        for missile in missiles:
            ...
            if missile:
                missile.kill()
                occur_explosion(screen, missile.rect.x, missile.rect.y)
                boss_hp -= 1
                shot_count += 200

        for lazer in lazers: # 레이저로 보스의 공격을 격추
            rock = lazer.collide(rocks)
            if rock:
                rock.kill() # 레이저는 관통 공격
                occur_explosion(screen, rock.rect.x, rock.rect.y)
        ...
        for rock in rocks:
            rock = fighter.collide(rocks) # 보스 공격에 맞았을 때
            if rock:
                rock.kill()
                count_life -= 1 # 목숨 - 1
